...
esp_err_t INA260_init(void) {
    return i2c_lucky_add_device(INA260_I2CADDR_DEFAULT, &ina260_dev_handle);
}
...
{
    uint8_t data[2];
    return i2c_lucky_register_read(ina260_dev_handle, INA260_REG_BUSVOLTAGE, data, 2) == ESP_OK;
}
...
float INA260_read_current(void)
{
    ...
    ESP_ERROR_CHECK(i2c_lucky_register_read(ina260_dev_handle, INA260_REG_CURRENT, data, 2));
...
float INA260_read_voltage(void)
{
    ...
    ESP_ERROR_CHECK(i2c_lucky_register_read(ina260_dev_handle, INA260_REG_BUSVOLTAGE, data, 2));
...
float INA260_read_power(void)
{
    ...
    ESP_ERROR_CHECK(i2c_lucky_register_read(ina260_dev_handle, INA260_REG_POWER, data, 2));
