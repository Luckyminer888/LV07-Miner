...
esp_err_t NVSDevice_parse_config(GlobalState * GLOBAL_STATE) {
    ...
    GLOBAL_STATE->miner_model_str = nvs_config_get_string(NVS_CONFIG_MINER_MODEL, "LV07");
    GLOBAL_STATE->asic_count = 1;
    GLOBAL_STATE->voltage_domain = 1;
    if (strcmp(GLOBAL_STATE->miner_model_str, "LV07") == 0) {
        GLOBAL_STATE->miner_model = MINER_LV07;
        GLOBAL_STATE->asic_count = 2;
    } else if (strcmp(GLOBAL_STATE->miner_model_str, "LV07 Pro") == 0) {
        GLOBAL_STATE->miner_model = MINER_LV07_PRO;
        GLOBAL_STATE->asic_count = 2;
    } else {
        GLOBAL_STATE->asic_count = -1;
    }
