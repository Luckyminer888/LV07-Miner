...
esp_err_t EMC2101_init(bool invertPolarity) {
    ...
    if (i2c_lucky_add_device(EMC2101_I2CADDR_DEFAULT, &emc2101_dev_handle) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to add device");
        return ESP_FAIL;
    }

    // set the TACH input
    ESP_ERROR_CHECK(i2c_lucky_register_write_byte(emc2101_dev_handle, EMC2101_REG_CONFIG, 0x04));

    if (invertPolarity) {
        ESP_ERROR_CHECK(i2c_lucky_register_write_byte(emc2101_dev_handle, EMC2101_FAN_CONFIG, 0b00100011));
    }
...
void EMC2101_set_fan_speed(float percent)
{
    ...
    ESP_ERROR_CHECK(i2c_lucky_register_write_byte(emc2101_dev_handle, EMC2101_REG_FAN_SETTING, speed));
}
...
uint16_t EMC2101_get_fan_speed(void)
{
    ...
    ESP_ERROR_CHECK(i2c_lucky_register_read(emc2101_dev_handle, EMC2101_TACH_LSB, &tach_lsb, 1));
    ESP_ERROR_CHECK(i2c_lucky_register_read(emc2101_dev_handle, EMC2101_TACH_MSB, &tach_msb, 1));
...
float EMC2101_get_external_temp(void)
{
    ...
    ESP_ERROR_CHECK(i2c_lucky_register_read(emc2101_dev_handle, EMC2101_EXTERNAL_TEMP_MSB, &temp_msb, 1));
    ESP_ERROR_CHECK(i2c_lucky_register_read(emc2101_dev_handle, EMC2101_EXTERNAL_TEMP_LSB, &temp_lsb, 1));
...
{
    uint8_t temp;
    ESP_ERROR_CHECK(i2c_lucky_register_read(emc2101_dev_handle, EMC2101_INTERNAL_TEMP, &temp, 1));
    return temp;
}
