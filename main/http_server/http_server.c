...
static int allow_update = 1;
static uint64_t check_uptime = 0;
...
static esp_err_t PATCH_update_settings(httpd_req_t * req)
{
    ...
    if ((item = cJSON_GetObjectItem(root, "minermodel")) != NULL) {
        nvs_config_set_string(NVS_CONFIG_MINER_MODEL, item->valuestring);
    }
...
static esp_err_t GET_system_info(httpd_req_t * req)
{
    ...
    cJSON_AddNumberToObject(root, "asicDetected", GLOBAL_STATE->asic_detected);
    ...
    cJSON_AddStringToObject(root, "minerModel", GLOBAL_STATE->miner_model_str);
    ...
    cJSON_AddNumberToObject(root, "fanSpeed", GLOBAL_STATE->POWER_MANAGEMENT_MODULE.fan_rpm);
    cJSON_AddNumberToObject(root, "allowUpdate", allow_update);
...
esp_err_t POST_WWW_update(httpd_req_t * req)
{
    ...
    if (allow_update != 1) {
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Upgrade not allowed");
        return ESP_FAIL;
    }
...
esp_err_t POST_OTA_update(httpd_req_t * req)
{
    ...
    nvs_config_set_u64(NVS_CONFIG_UP_TIME, check_uptime);
