...
void SYSTEM_init_system(GlobalState * GLOBAL_STATE)
{
    ...
    module->up_time = nvs_config_get_u64(NVS_CONFIG_UP_TIME, 0);
...
void SYSTEM_init_peripherals(GlobalState * GLOBAL_STATE) {
    ...
    EMC2302_set_fan_speed(0, nvs_config_get_u16(NVS_CONFIG_FAN_SPEED, 60));
    EMC2302_set_fan_speed(1, nvs_config_get_u16(NVS_CONFIG_FAN_SPEED, 60));
...
void SYSTEM_notify_new_ntime(GlobalState * GLOBAL_STATE, uint32_t ntime)
{
    ...
    uint64_t uptime = nvs_config_get_u64(NVS_CONFIG_UP_TIME, 0);
    ESP_LOGI(TAG, "uptime %u", (unsigned int) uptime);
    if (uptime < ntime) {
        uint64_t ntime_64 = (uint64_t) ntime;
        nvs_config_set_u64(NVS_CONFIG_UP_TIME, ntime_64);
    }
    ...
    switch (GLOBAL_STATE->device_model) {
        ...
        case DEVICE_GAMMA:
            if (OLED_status()) {
                ...
                OLED_clearLine(1);
                OLED_writeString(0, 1, "See settings");
                OLED_clearLine(2);
                OLED_clearLine(3);
    ...
    switch (GLOBAL_STATE->device_model) {
        ...
        case DEVICE_GAMMA:
            if (OLED_status()) {
                ...
                memset(module->oled_buf, 0, 20);
                snprintf(module->oled_buf, 20, "Gh: %.0f W/Th: %.1f", module->current_hashrate, efficiency);
