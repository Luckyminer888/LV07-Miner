...
static uint8_t TPS546_I2CADDR = 0x24;
static uint8_t TPS546_INDEX = 0;
//static uint8_t TPS546_ADDRESSES[3] = {0x24, 0x7F, 0x14};  // 假设三个TPS546地址
static i2c_master_dev_handle_t tps546_dev_handles[3];      // 每个TPS546设备一个句柄
...
{
    return i2c_lucky_register_read(tps546_dev_handles[TPS546_INDEX], command, data, 1);
}
...
{
    return i2c_lucky_register_write_byte(tps546_dev_handles[TPS546_INDEX], command, data);
}
...
{
    uint8_t data[2];
    if (i2c_lucky_register_read(tps546_dev_handles[TPS546_INDEX], command, data, 2) != ESP_OK) {
        return ESP_FAIL;
    } else {
...
{
    return i2c_lucky_register_write_word(tps546_dev_handles[TPS546_INDEX], command, data);
}
...
static esp_err_t smb_read_block(uint8_t command, uint8_t *data, uint8_t len)
{
    ...
    if (i2c_lucky_register_read(tps546_dev_handles[TPS546_INDEX], command, buf, len+1) != ESP_OK) {
        free(buf);
        return ESP_FAIL;
    }
...
static esp_err_t smb_write_block(uint8_t command, uint8_t *data, uint8_t len)
{
    ...
    //write it all
    if (i2c_lucky_register_write_bytes(tps546_dev_handles[TPS546_INDEX], buf, len+2) != ESP_OK) {
        free(buf);
        return ESP_FAIL;
    } else {
        free(buf);
...
void TPS546_set_addr(uint8_t addr) {
    if (addr == 0x24) {
        TPS546_INDEX = 0;
    } else if (addr == 0x7F) {
        TPS546_INDEX = 1;
...
int TPS546_init(void)
{
    ...
    ESP_LOGI(TAG, "0x%02X Initializing the core voltage regulator", TPS546_I2CADDR);

    if (i2c_lucky_add_device(TPS546_I2CADDR, &tps546_dev_handles[TPS546_INDEX]) != ESP_OK) {
